...
author: "Lex Comber, Chris Brunsdon,  Paul Harris, Binbin Lu, Nick Malleson"
date: "September 2021"
output:
...
#df %>% filter(Year != "2021") %>% group_by(Year) %>% 
#  summarise(`Annual Citation Count` = sum(Cited.by, na.rm = T)) %>%
#  ggplot(mapping = aes(x = Year, y = `Annual Citation Count`)) + 
...
```{r tab1, echo= F}
df = read.csv("tab1_v2.csv")
names(df) = gsub("\\.", " ", names(df))
index = t(apply(df, 1, function(x) is.na(x)))
df[index] = "-"

knitr::kable(df,
             caption = "GWmodel package functionality by the main groups of related functions (* for mean/median only) where applicable.",
...
+ `gwda`: This function implements GW discriminant analysis.
